...
import type { IIngredientCreateInputs } from "./IngredientInterface";

const schema: yup.ObjectSchema<IIngredientCreateInputs> = yup
  .object({
    name: yup
      .string()
      .required("Назва інгредієнта обов’язкова")
...
const IngredientCreatePage: React.FC = () => {
  const navigate = useNavigate();

  const { register, handleSubmit, formState: { errors }, watch } = useForm<IIngredientCreateInputs>({
  ...
  const onSubmit = async (data: IIngredientCreateInputs) => {
    try {
      ...
      await axios.post(`${EnvConfig.API_URL}/api/Ingredient/create`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      navigate("/admin/ingredients");
    } catch (error) {
      console.error("Помилка при створенні інгредієнта:", error);
      alert("Не вдалося створити інгредієнта. Спробуйте пізніше.");
    }
  };

  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow">
      <h1 className="text-2xl font-bold mb-4">Створити інгредієнт</h1>
